            ...
            <tbody>
              <!-- <div v-for="(list, i) in PROD_PREPARATION_LIST" :key="i">
                <tr class="data-table-data-row ds-pack-tt-mio-tr">
                  <td colspan="4" v-if="checkPPIsPresent(list)"><p class="td-ds-pack-tt-mio">Warehouse:<span>{{ getTTNameFromPPList(list[0]) }}</span></p></td>
                </tr>
              ...
              <tr v-for="(schedule, j) in INVOICE_ID_FROM_LEFT ? INVOICE_ID_FROM_LEFT.batch_info : 0" :key="j" class="data-table-data-row">
                <td>{{ j }}</td>
                <td style="font-weight: 500; text-align: left;">{{ schedule.transfer_detail.prod_name }}</td>
